...
const _MOUSE_ITEM_IMG =
  "<img class='mouse' src='imgs/mouse.png'width='16' height='16'/>";
const _CAT_ITEM_IMG = function (row, column) {
  return `<img src='imgs/cat.png' class='cat cat-${row}-${column}' width='16' height='16'/>`;
};
const _MOUSE_WHEEL_IMG =
  "<img src='imgs/MOUSE_WHEEL.png' width='16' height='16'/>";
const _ROTTEN_CHEESE_IMG =
  "<img src='imgs/ROTTEN_CHEESE.png' width='16' height='16'/>";
...
class CheesyPursuit {
  ...
  resetBoardAfterBeingCaught() {
    ...
    this.stopTimers();
    this.removeCats();
    this.settings.set("lives", parseInt(this.settings.get("lives")) - 1);
  ...
  livesBox() {
    return `<div class='lives top'><img src='imgs/mouse.png'width="12"><span class="livescounter">${this.settings.get(
      "lives"
    )}</span></div>`;
  }
  ...
  rottenCheeseBox() {
    return `<div class='ROTTEN_CHEESE top'>${_ROTTEN_CHEESE_IMG} ${this.settings.get(
      "ROTTEN_CHEESE"
    )}</div>`;
  }
  mouseWheelBox() {
    return `<div class='MOUSE_WHEEL top'>${_MOUSE_WHEEL_IMG} ${this.settings.get(
      "MOUSE_WHEEL"
    )}</div>`;
  }
  ...
  addScore(nextPosition) {
    ...
    if (GRID[nextPosition[0]][nextPosition[1]] === CHEESE) {
      ...
      var new_score =
        1 * (parseInt(this.settings.get("MOUSE_WHEEL")) || 1) + current_score;
  ...
  checkForCat(nextPosition) {
    if (this.getIndex(MOUSE) === this.getIndex(CAT)) {
      this.gameOver();
    } else {
      return true;
    }
  ...
  checkifCatnearby(location) {
    for (let cat in CATS) {
      ...
      if (get_cat_location) {
        if (
          this.arraysMatch(location, [get_cat_location[0], get_cat_location[1]])
        ) {
